...
export const usePollarAuthBridge = () => {
	...
	const bridgeToWardFundSession = useCallback(
	...
	const tryAutoBridge = useCallback(
		(linkToExistingUserId?: string) => {
			...
			void bridgeToWardFundSession(linkToExistingUserId ? { linkToExistingUserId } : undefined).catch(
				() => {
					// Errors are surfaced via toast in bridgeToWardFundSession.
				},
			)
		},
		[bridgeToWardFundSession, getClient, session?.user?.id, session?.user?.onboardingProvider],
	)
	...
	const linkPollarToCurrentAccount = useCallback(async () => {
		...
		loginStartedRef.current = true
		await bridgeToWardFundSession({ linkToExistingUserId: session.user.id })
	}, [bridgeToWardFundSession, session?.user?.id])
