 ...
 *
 * Evolve (upgrade tier of) an existing WardFund Kinder NFT.
	...
	try {
		...
		try {
			...
			const uploadResult = await uploadFileToIPFS(
				svgBuffer,
				`wardfund-kinder-${newTier}-${existingNFT.token_id}.svg`,
				'image/svg+xml',
			)
			...
			imageUri = `https://wardfund.org/images/nft-${newTier}.svg`
		}
		...
		try {
			const metaResult = await uploadMetadataToIPFS(
				nftMetadataJSON,
				`wardfund-kinder-metadata-${existingNFT.token_id}-${newTier}`,
			)
