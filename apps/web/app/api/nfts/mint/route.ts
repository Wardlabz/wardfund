 ...
 * POST /api/nfts/mint
 *
 * Mint a new WardFund Kinder NFT for a user.
	...
	try {
		...
		try {
			...
			const uploadResult = await uploadFileToIPFS(
				svgBuffer,
				`wardfund-kinder-${tier}-${userId.slice(0, 8)}.svg`,
				'image/svg+xml',
			)
			...
			console.warn('[NFT Mint] Failed to upload image to Pinata, using placeholder:', err)
			imageUri = `https://wardfund.org/images/nft-${tier}.svg`
		}
		...
		try {
			const metaResult = await uploadMetadataToIPFS(
				nftMetadataJSON,
				`wardfund-kinder-metadata-${userId.slice(0, 8)}`,
			)
