...
Isolated C-address (Soroban contract) wallet implementation for WardFund. **Suspended on Mainnet production** until Trustless Work supports Smart Account Kit signing with C-addresses.
...
- WardFund auth factory: `FACTORY_CONTRACT_ID`, `CONTROLLER_CONTRACT_ID`
...
- Server-side C-address deployment uses **WardFund factory contracts** (`StellarPasskeyService`), not `kit.createWallet()`.
...
7. Enable on Mainnet only after TW + WardFund validation complete.
