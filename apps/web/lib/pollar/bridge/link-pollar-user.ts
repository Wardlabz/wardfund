...
/**
 * Finds or creates a WardFund profile for a verified Pollar session.
 */
export const linkPollarUserToWardFund = async (
	verified: PollarVerifiedSession,
	options: { linkToExistingUserId?: string } = {},
): Promise<LinkedPollarProfile> => {
	const email = verified.email?.trim().toLowerCase()
	if (!email) {
		throw new Error('Pollar account must include an email to link with WardFund')
	}
