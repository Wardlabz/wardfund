# Pollar Integration (WardFund)

Isolated module for [Pollar](https://docs.pollar.xyz) social/email wallet onboarding bridged into WardFund's NextAuth session.
...
- `WardFundPollarProvider` wraps the app with `@pollar/react/styles.css` imported
...
2. **Code fix (already applied):** `WardFundPollarProvider` passes a **singleton** `PollarClient` instance (not a config object) so remounts do not create competing refresh loops.
...
1. **Pollar provider remounting** — keep `WardFundPollarProvider` outside `SessionProvider` so NextAuth key changes do not recreate the SDK client.
...
### User completes Pollar login but WardFund session is missing
...
- Pollar account includes an email (required for WardFund profile linking).
