...
export async function uploadMetadataToIPFS(
	...
	const response = await fetch(`${PINATA_API_URL}/pinning/pinJSONToIPFS`, {
		...
		body: JSON.stringify({
			...
			pinataMetadata: {
				name: name || `wardfund-nft-${Date.now()}`,
			},
		}),
	})
...
export const TIER_CONFIGS: Record<NFTTier, TierConfig> = {
	bronze: {
		name: 'Bronze Kinder',
		description:
			'A Bronze-tier Kinder NFT, recognizing your first steps in making an impact through WardFund.',
		minPoints: 0,
		governanceVotes: 1,
		color: '#CD7F32',
	},
	silver: {
		name: 'Silver Kinder',
		description:
			'A Silver-tier Kinder NFT, celebrating your growing commitment to positive change on WardFund.',
		minPoints: 100,
		governanceVotes: 3,
		color: '#C0C0C0',
	},
	gold: {
		name: 'Gold Kinder',
		description:
			'A Gold-tier Kinder NFT, honoring your significant contributions and consistent giving on WardFund.',
		minPoints: 500,
		governanceVotes: 5,
		color: '#FFD700',
	},
	diamond: {
		name: 'Diamond Kinder',
		description:
			'A Diamond-tier Kinder NFT, the highest honor for exceptional impact and community leadership on WardFund.',
		minPoints: 2000,
		governanceVotes: 10,
		color: '#B9F2FF',
	},
...
export function generateTierSVG(tier: NFTTier, tokenId: number): string {
  ...
  <text x="200" y="370" text-anchor="middle" fill="#999" font-size="10" font-family="sans-serif">WardFund Reputation NFT</text>
</svg>`
}
...
export function buildNFTMetadata(
	...
	return {
		...
		image: imageUri,
		external_url: 'https://wardfund.org/profile?section=gamification',
		attributes: [
			{
				trait_type: 'Tier',
