			...
			if (hasUserInfo && isEtherfuseClientNotLinkedError(error.message)) {
				try {
					...
					if (
						...
						throw new AppError(
							'Your email is already linked to a separate Etherfuse account outside WardFund. Complete onboarding in the opened Etherfuse page using a different email, or contact Etherfuse support to link your account to WardFund.',
							409,
						)
					}
