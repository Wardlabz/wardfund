...
import { z } from 'zod'
import { linkPollarUserToWardFund } from '~/lib/pollar/bridge/link-pollar-user'
...
export interface WardFundPollarCredentials {
	pollarSessionJson: string
	linkToExistingUserId?: string
}

export const wardfundPollarProvider = CredentialsProvider({
	id: 'pollar',
	name: 'Pollar',
	credentials: {
		pollarSessionJson: { label: 'Pollar Session', type: 'text' },
		linkToExistingUserId: { label: 'Link To User ID', type: 'text' },
	} as Record<keyof WardFundPollarCredentials, CredentialInput>,
	async authorize(credentialsArg, _req): Promise<User | null> {
		const credentials = credentialsArg as WardFundPollarCredentials | undefined
		...
		try {
			...
			const linked = await linkPollarUserToWardFund(verified, {
