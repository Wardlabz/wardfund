...
export const wardfundWebAuthnProvider = CredentialsProvider({
	name: 'Credentials',
	credentials: {
	...
	} as Record<keyof WardFundWebAuthnCredentials, CredentialInput>,
	async authorize(credentialsArg, _req): Promise<User | null> {
		const credentials = credentialsArg as WardFundWebAuthnCredentials | undefined
...
export interface WardFundWebAuthnCredentials {
	userId: string
	pubKey: string
	credentialId: string
	address: string
