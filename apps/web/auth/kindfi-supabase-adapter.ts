...
interface WardFundUser extends AdapterUser {
	device?: DeviceData
	userData?: UserData
}

/**
 * Custom Supabase adapter for WardFund WebAuthn authentication
 * Extends the standard Supabase adapter to handle WebAuthn credentials
 * and maintain device/user data synchronization
 */
export function WardFundSupabaseAdapter(): Adapter {
	...
	return {
		...
		async getUser(id: string) {
			...
			try {
				...
				// Enhance user with WardFund-specific data
		...
		async getUserByAccount({
          ...
          `,
				...
				const { data: profileData } = await supabase
					...
					.single()
				const userData = nextAuthUser as unknown as WardFundUser
				const wardfundUser: WardFundUser = {
					id: userData.id,
		...
		async getSessionAndUser(sessionToken: string) {
			...
			if (result?.user) {
				...
				if (profileData) {
					;(result.user as WardFundUser).userData = profileData as UserData
				}
			}
