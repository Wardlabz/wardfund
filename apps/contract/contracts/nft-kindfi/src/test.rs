...
use crate::{WardFundNFT, WardFundNFTClient};

/// Helper to create a test environment with an initialized contract.
struct TestEnv {
    env: Env,
    admin: Address,
    client: WardFundNFTClient<'static>,
}
...
impl TestEnv {
    fn new() -> Self {
        ...
        let name = String::from_str(&env, "WardFund NFT");
        let symbol = String::from_str(&env, "KFNFT");
        let base_uri = String::from_str(&env, "https://api.wardfund.org/nft/");

        let contract_id = env.register(WardFundNFT, (&admin, name, symbol, base_uri));
        let client = WardFundNFTClient::new(&env, &contract_id);
    ...
    fn create_metadata(&self, name: &str) -> NFTMetadata {
        NFTMetadata {
            ...
            description: String::from_str(&self.env, "A WardFund NFT"),
            image_uri: String::from_str(&self.env, "https://example.com/image.png"),
            external_url: String::from_str(&self.env, "https://wardfund.org"),
...
fn test_constructor_sets_metadata() {
    ...
    assert_eq!(test.client.name(), String::from_str(&test.env, "WardFund NFT"));
...
fn test_mint_with_metadata() {
    ...
    let metadata = test.create_metadata("WardFund NFT #0");
...
fn test_mint_sequential_token_ids() {
    ...
    let metadata1 = test.create_metadata("WardFund NFT #0");
    let metadata2 = test.create_metadata("WardFund NFT #1");
    let metadata3 = test.create_metadata("WardFund NFT #2");
...
fn test_update_metadata() {
    ...
    let updated_metadata = NFTMetadata {
        ...
        external_url: String::from_str(&test.env, "https://wardfund.org/updated"),
        attributes: soroban_sdk::vec![
            &test.env,
            NFTAttribute {
