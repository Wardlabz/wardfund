# WardFund NFT Contract

A non-fungible token (NFT) contract for the WardFund platform's reputation and incentivization system. Built on OpenZeppelin Stellar Contracts v0.6.0 for Soroban.
...
- `name`: Collection name (e.g., "WardFund NFT")
- `symbol`: Collection symbol (e.g., "KFNFT")
- `base_uri`: Base URI for token metadata (e.g., "https://api.wardfund.org/nft/")
...
use nft_wardfund::{WardFundNFT, NFTMetadata};

// 1. Initialize contract
WardFundNFT::__constructor(
    &env,
    admin,
    String::from_str(&env, "WardFund NFT"),
    String::from_str(&env, "KFNFT"),
    String::from_str(&env, "https://api.wardfund.org/nft/")
);

// 2. Grant minter role
WardFundNFT::grant_role(&env, minter_address, symbol_short!("minter"), admin);

// 3. Grant metadata_manager role
let metadata_role = WardFundNFT::metadata_manager_role(&env);
WardFundNFT::grant_role(&env, manager_address, metadata_role, admin);

// 4. Mint NFT with metadata
let metadata = NFTMetadata {
    name: String::from_str(&env, "Early Supporter Badge"),
    description: String::from_str(&env, "Badge awarded to early WardFund supporters"),
    image_uri: String::from_str(&env, "https://api.wardfund.org/images/badge.png"),
    external_url: String::from_str(&env, "https://wardfund.org/badges/early"),
    attributes: vec![
        &env,
...
let token_id = WardFundNFT::mint_with_metadata(&env, minter, recipient, metadata);
// token_id = 0 (first token)

// 5. Query metadata
let stored_metadata = WardFundNFT::get_metadata(&env, token_id);

// 6. Query owner
let owner = WardFundNFT::owner_of(&env, token_id);

// 7. Transfer NFT
WardFundNFT::transfer(&env, owner, new_owner, token_id);
...
let new_metadata = NFTMetadata {
    ...
    image_uri: String::from_str(&env, "https://api.wardfund.org/images/badge-v2.png"),
    external_url: String::from_str(&env, "https://wardfund.org/badges/early-v2"),
    attributes: vec![
        &env,
...
WardFundNFT::update_metadata(&env, manager, token_id, new_metadata);

// 9. Burn NFT (requires burner role)
WardFundNFT::grant_role(&env, burner_address, symbol_short!("burner"), admin);
WardFundNFT::burn(&env, owner, token_id);
```
...
```bash
cd apps/contract
cargo build --target wasm32-unknown-unknown --release -p nft-wardfund
```
...
```bash
cd apps/contract
cargo test -p nft-wardfund
```

## License

MIT License - WardFund Org
