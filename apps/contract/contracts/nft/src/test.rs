#![cfg(test)]
use crate::{
    access::{METADATA_MANAGER_ROLE, MINTER_ROLE},
    pausable::PauseOperation,
    types::NFTDetail,
};
use crate::{errors::NFTError, WardFundNFT};
use soroban_sdk::{
    auth::Signature,
...
// Test helper function to create a test environment with WardFundNFT contract
fn setup_test() -> (Env, WardFundNFT, Address, Address) {
    let env = Env::default();
    let contract_id = env.register_contract(None, WardFundNFT);
    let admin = Address::random(&env);
    let user = Address::random(&env);

    // Initialize the contract
    WardFundNFT::client(&env, &contract_id)
        .initialize(&admin)
        .unwrap();

    (env, WardFundNFT::client(&env, &contract_id), admin, user)
}
...
fn test_initialize() {
    ...
    assert!(
        WardFundNFT::client(&env, &env.register_contract(None, WardFundNFT))
...
fn test_mint_with_minter_role() {
    ...
    env.mock_auth()
        .with_no_verification()
        .with_address(admin.clone())
        .run(|| {
...
fn test_role_management() {
    ...
    env.mock_auth()
        ...
        .run(|| {
            contract
                .grant_role(&MINTER_ROLE, &user, &Vec::new(&env))
                .unwrap();
        });
    ...
    env.mock_auth()
        ...
        .run(|| {
            contract
                .revoke_role(&MINTER_ROLE, &user, &Vec::new(&env))
                .unwrap();
        });
...
fn test_metadata_update() {
    ...
    assert_eq!(
        token_detail.metadata.description,
        String::from_str(&env, "Updated Description")
    );
}
...
fn test_rate_limiting() {
    ...
    env.mock_auth()
        ...
        .run(|| {
            contract
                .set_rate_limit(&String::from_str(&env, "mint"), &1, &1000)
                .unwrap();
        });
