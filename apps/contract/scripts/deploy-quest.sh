#!/bin/bash
# deploy-quest.sh
# Deployment script for WardFund Quest Contract

set -e

echo "========================================"
echo "  WardFund Quest Contract Deployment"
echo "========================================"
...
QUEST_CONTRACT_ID=$(stellar contract deploy \
    --network "$NETWORK" \
    --source "$SOURCE" \
    --wasm-hash "$QUEST_WASM_HASH" \
    -- \
    --admin "$ADMIN_ADDRESS" \
    --reputation_contract "$REPUTATION_CONTRACT_ID")

echo "Contract ID: $QUEST_CONTRACT_ID"
...
DEPLOYMENT_FILE="quest-deployment-info-${NETWORK}.txt"
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

cat > "$DEPLOYMENT_FILE" << EOF
=== WardFund Quest Contract Deployment ===
Network:         $NETWORK
Deployment Time: $TIMESTAMP

=== Contract Information ===
WASM Hash:       $QUEST_WASM_HASH
Contract ID:     $QUEST_CONTRACT_ID
Admin:           $ADMIN_ADDRESS
...
# Grant admin role (can create quests):
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- grant_role --account <ADMIN_ADDRESS> --role 'admin' --caller $ADMIN_ADDRESS

# Grant recorder role (can update quest progress):
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- grant_role --account <RECORDER_ADDRESS> --role 'recorder' --caller $ADMIN_ADDRESS

=== Example Quest Creation ===
# Create a multi-region donation quest:
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- create_quest \
  --caller $ADMIN_ADDRESS \
  --quest_type 0 \
  --name "Donate to 3 Regions" \
  --description "Support campaigns in 3 different regions" \
  --target_value 3 \
  --reward_points 30 \
  --expires_at 0
...
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- get_quest --quest_id 0

# Get user quest progress:
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- get_user_quest_progress --user <USER_ADDRESS> --quest_id 0

# Update quest progress (requires recorder role):
stellar contract invoke --network $NETWORK --source $SOURCE --id $QUEST_CONTRACT_ID -- update_progress \
...
echo ""
echo "========================================"
echo "  Quest Contract Deployment Complete!"
echo "========================================"
echo ""
echo "Network:     $NETWORK"
echo "WASM Hash:   $QUEST_WASM_HASH"
...
echo ""
echo "Next steps:"
echo "1. Grant 'admin' role to addresses that should create quests"
echo "2. Grant 'recorder' role to addresses that should update quest progress"
echo "3. Create initial quests using create_quest function"
echo ""
