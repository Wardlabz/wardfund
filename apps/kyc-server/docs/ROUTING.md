...
This document explains the routing implementation in the WardFund KYC Server application, which combines server-side rendering (SSR) with client-side routing.
    ...
    async GET(req: Request) {
      ...
      return withConfiguredCORS(async () => {
        const stream = await renderToReadableStream(
          <Home message="Welcome to WardFund KYC Server!" />,
        );
    ...
    children: [
      {
        ...
        element: <div className="card">
          <p>Welcome to WardFund KYC Server!</p>
