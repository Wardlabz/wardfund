# WardFund Monorepo — Agent Guide

Executor context for AI agents working in this repository. Read this before making changes.

## Overview

WardFund is a Bun workspaces monorepo for blockchain-powered social-impact crowdfunding on Stellar/Soroban. The primary app is `apps/web` (Next.js 16). Shared logic lives in `packages/*`; backend services in `services/*`.
...
| `apps/indexer` | `@wardfund/indexer` | SubQuery indexer for on-chain activity → off-chain data |
