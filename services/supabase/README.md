# Supabase Service - WardFund

This document provides instructions for developers to set up and run the Supabase service locally within the WardFund monorepo.
...
Update the `.env` file with the WardFund Supabase project id. This is for remote type generation and to identify the local development host.
...
| Sender email | `noreply@wardfund.org` |
| Sender name | `WardFund Notifications` |

3. Ensure `wardfund.org` is verified in [Resend](https://resend.com/domains) and `noreply@wardfund.org` is allowed as a sender.
